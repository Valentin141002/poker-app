...
function renderMatch2Details(matchID, detailsRow) {
  ...
  if (g.waitingCount !== undefined && !g.players) {
    cell.innerHTML = `<div><strong>En attente :</strong> ${g.waitingCount}/2 joueurs</div>`;
    return;
  }
  cell.innerHTML = `
    <div style="padding:8px;">
...
tabGames2.addEventListener('click', () => {
  ...
  tableForm.querySelectorAll('input[name="player"]').forEach(i=>i.addEventListener('input',checkTableInputs));
  checkTableInputs();
  tableForm.addEventListener('submit', async e => {
    e.preventDefault();
    const players = Array.from(tableForm.elements['player']).map(i => i.value.trim());
    try {
      ...
      result10.innerHTML = `<p>Lien : <a href="${data.link}" target="_blank">${data.link}</a></p>`;
  
      // ici on surligne la nouvelle table
      const newId = new URL(data.link).searchParams.get('table');
      fetchTables(newId);
    } catch(err){
  ...
  const matchForm  = document.getElementById('match2-form'),
        create2Btn  = document.getElementById('create2-btn'),
  ...
  matchForm.querySelectorAll('input[name="player2"]').forEach(i=>i.addEventListener('input',checkMatchInputs));
  checkMatchInputs();
  matchForm.addEventListener('submit', async e=>{
    e.preventDefault();
    const players = Array.from(matchForm.elements['player2']).map(i=>i.value.trim());
    try {
      ...
      const data = await res.json();
      if(!res.ok) throw new Error(data.error||res.statusText);
      result2.innerHTML = `<p>Lien : <a href="${data.link}" target="_blank">${data.link}</a></p>`;
      fetchMatches();
    } catch(err){ alert('Erreur création match : '+err.message); }
  });
  ...
  async function fetchTables() {
    ...
    // 1) IDs déjà affichées
    previousTableIDs = Array.from(tbody.querySelectorAll('tr[data-table]'))
    ...
    try {
      ...
      const list = await res.json();  // [{ tableID, mode, players, winner? }, …]
      // 3) On repère les nouvelles tables
      const newIDs = list
        .map(t => t.tableID)
        .filter(id => !previousTableIDs.includes(id));
      ...
      list.forEach(t => {
        ...
        copyBtn.addEventListener('click', () => {
          const url = `${location.origin}?table=${t.tableID}`;
        ...
        tr.querySelector('.delete-btn').addEventListener('click', async () => {
          ...
          try {
            const del = await fetch(`/admin/deleteTable/${t.tableID}`, { method: 'DELETE' });
            if (!del.ok) throw new Error(await del.text());
            const det = tr.nextElementSibling;
            if (det?.classList.contains('details-row')) det.remove();
            tr.remove();
          } catch (err) {
  ...
  async function fetchMatches() {
    ...
    // 1) IDs déjà affichés
    previousMatchIDs2 = Array.from(tbody.querySelectorAll('tr[data-match]'))
                              .map(r => r.dataset.match);
    ...
    try {
      ...
      const list = await res.json();  // [{ matchID, mode, players, winner? }, …]
      // 3) Nouvelles entrées
      const newIDs = list
        .map(m => m.matchID)
        .filter(id => !previousMatchIDs2.includes(id));
      ...
      list.forEach(m => {
        ...
        const copyBtn = tr.querySelector('.copy-btn');
        copyBtn.addEventListener('click', () => {
          const url = `${location.origin}?match2=${m.matchID}`;
          navigator.clipboard.writeText(url)
            .then(() => {
              const msg = document.createElement('span');
              msg.className = 'copy-feedback';
        ...
        tr.querySelector('.delete-btn').addEventListener('click', async () => {
          ...
          try {
            const del = await fetch(`/admin/deleteMatch2/${m.matchID}`, { method: 'DELETE' });
            if (!del.ok) throw new Error(await del.text());
            const det = tr.nextElementSibling;
            if (det?.classList.contains('details-row')) det.remove();
            tr.remove();
      ...
      tr.innerHTML = `
        <td colspan="5" style="text-align:center;color:red">
          Échec du chargement des matches.
        </td>`;
      tbody.appendChild(tr);
    }
  }  
